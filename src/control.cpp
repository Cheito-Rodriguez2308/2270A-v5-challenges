...
void driver_control_loop() {
  ...
  while (true) {
    ...
    double sens = s.turbo_on ? cfg.SENSITIVITY_TURBO : cfg.SENSITIVITY_SOFT;

    double f_shaped = shape_axis(f_raw, sens, deadbandFwd);
    double t_shaped = shape_axis(t_raw, sens, deadbandTurn);
    ...
    if (s.turbo_on) {
      ...
      s.t_slew = (slewTurnTurbo == 0)
        ? s.t_target
        : apply_slew(s.t_target, s.t_slew, slewTurnTurbo);
      ...
      s.t_slew = (slewTurnNormal == 0)
        ? s.t_target
        : apply_slew(s.t_target, s.t_slew, slewTurnNormal);
    ...
    if (lastTurbo != turboFlag || voltage10 != lastVoltage10 ||
        comp100 != lastComp100) {
      char buf0[32];
      char buf1[32];
