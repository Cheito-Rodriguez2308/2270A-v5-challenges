...
void routine_match_der() {
  ...
  const int FWD2_MM = mm_from_in(9.5);        // approach after piston
  const int FWD3_MM = mm_from_in(1.0);        // small tap forward
  const int BACK_MM = mm_from_in(25.5);       // retreat to intake cycle
  ...
  // ==========================================================================
  turn_imu_deg_2stage(
    90,                                      // right side routine turns opposite direction
    turn_fast,
    turn_slow,
    0.88,
    160,
  ...
  drive_straight_mm(
    FWD2_MM,
    autopct(85, comp),                       // more punch-in on this side
    kP_h, kI_h, kD_h,
    110.0,
    pros::E_MOTOR_BRAKE_HOLD,
    80,
